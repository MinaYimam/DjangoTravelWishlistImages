...
def place_was_visited(request, place_pk):
    if request.method == 'POST':
        ...
        if place.user == request.user:          #check if the user is log in, can be able to make the request
            place.visited = True
            place.save()
        else:
...
def place_details(request, place_pk):
    ...
    # if POST request, validate from data and update
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.erorr(request, form.errors)
        return redirect('place_details', place_pk=place_pk)
    else:
        # if GET request, show Place info and optional form
        # if place visited, show form, is place is not visited, no form
        if place.visited:
